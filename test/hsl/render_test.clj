...
(deftest render-caps-and-labels-alerts
  (testing "<=3 alerts: all shown, no overflow marker"
    (let [html (render/render-full (board-with ["a" "b" "c"]))]
      (is (str/includes? html "Häiriötiedotteet"))
      (is (not (str/includes? html "muu")))))
  (testing "4 alerts: 3 shown + singular marker"
    (let [html (render/render-full (board-with ["a" "b" "c" "d"]))]
      (is (str/includes? html "+1 muu häiriötiedote"))
      (is (not (str/includes? html "muuta")))))
  (testing "6 alerts: 3 shown + plural marker"
    (let [html (render/render-full (board-with ["a" "b" "c" "d" "e" "f"]))]
