...
```
 ...
 Digitransit GraphQL ──▶│ bb server (http-kit) :4001                 │
   (api.digitransit.fi) │   in-process TTL cache (atom)              │
                        │   GET /api/trmnl → {markup, markup_half_…} │
                        │   GET /preview   → full HTML (development) │
...
curl -s localhost:4001/api/trmnl | jq 'keys' # Returns the four markup keys
open http://localhost:4001/preview           # Eyeball the full board in a browser
```
...
### The board: which stops, which routes

Everything about what the board shows lives in
[`src/hsl/config.clj`](src/hsl/config.clj). The board has a two-column layout;
...
2. Set the Polling URL to your server's `/api/trmnl` endpoint (through your
   reverse proxy; e.g. `https://board.example.com/api/trmnl`).
